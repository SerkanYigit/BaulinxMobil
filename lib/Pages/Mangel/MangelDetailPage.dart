...
class MangelDetailPage extends StatefulWidget {
  GenericTodo msg;
  int? ownerId;
  ...
  @override
  State<MangelDetailPage> createState() => MangelDetailPageState();
}

class MangelDetailPageState extends State<MangelDetailPage> {
  String Base64Image = "";
  ...
  void initState() {
    ...
    titleController.text = widget.msg.content!;
    // _replyTextcontroller.setText(widget.msg.description!);
    startDate.value = DateTime.parse(widget.msg.startDate!);
    endDate.value = DateTime.parse(widget.msg.endDate!);
    statusValue = widget.msg.status;

    selectedLabelIndexes = widget.msg.labelList!
        .map((e) => widget.cboLabelsList.indexWhere((element) =>
            element.value.toString().split("+").first == e.labelTitle))
        .toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("Mängel ${widget.msg.id}"),
      ),
      body: ConstrainedBox(
          ...
          child: Card(
            ...
            child: Column(
              children: [
                  ...
                  child: Listener(
                    ...
                    child: HtmlEditor(
                      ...
                      htmlEditorOptions: HtmlEditorOptions(
                        ...
                        initialText: widget.msg.description!,
                        // initialText,
                      ),
                ...
                Tags(
                  itemCount:
                      //widget.msg.labelList!.length, //required
                      labelItems.length,
                  itemBuilder: (i) {
                    return ItemTags(
                      ...
                      index: i,
                      title:
                          //widget.msg.labelList![i].labelTitle!,
                          labelItems[i],
                            ...
                            children: [
                              ...
                              ValueListenableBuilder(
                                ...
                                builder: (context, data, _) {
                                  return Text(
                                    BoardDateFormat(
                                        DateTimePickerType.date.formatter(
                                      withSecond: DateTimePickerType.time ==
                                          DateTimePickerType.date,
                                    )).format(data),
                      ...
                      CustomIconWithBackground(
                        ...
                        onPressed: () async {
                          ...
                          try {
                            ...
                            await _todoDB.UpdateCommonTodos(
                              _controllerDB.headers(),
                              UserId: _controllerDB.user.value!.result!.id!,
                              CustomerId: null,
                              CommonBoardId: ownerId,
                              TodoName: titleController.text,
                              Description: htmlMessageSubject,
                              //takenoteController.text,
                              ...
                              Status: statusValue,
                              TodoId: widget.msg.id,
                            ).then((value) async {
                              print(value);
                              await InsertTodoLabelList(selectedLabels,
                                      value.genericTodoUpdate!.id!)
                                  .then((value2) {
                                print("CHECHER>kedddd" +
                                    value2.Result.value[0].labelId.toString());

                                checkedItems.isNotEmpty
                                    ? checkedItems.forEach((element) async {
                                        await InsertOrUpdateTodoCheckList(
                                            0, //       genericTodo.checkList!.checkListItem![i].id!,
                                            value.genericTodoUpdate!.id!,
                                            element.text.text,
                                            element.checked);
                                      })
  ...
  Future InsertTodoLabelList(List<int> LabelIds, int TodoId) async {
    return await _controllerLabel.InsertTodoLabelList(_controllerDB.headers(),
            TodoId: TodoId,
            LabelIds: LabelIds,
            UserId: _controllerDB.user.value!.result!.id!)
        .then((value) {
