...
class UserEmailCreate extends StatefulWidget {
  const UserEmailCreate({Key? key}) : super(key: key);

  @override
  _UserEmailCreateState createState() => _UserEmailCreateState();
}

class _UserEmailCreateState extends State<UserEmailCreate> {
  bool isLoading = true;
  ...
  getEmailTypeList() async {
    await _controllerUser.GetEmailTypeList(_controllerDB.headers(),
      ...
      setState(() {
        ...
        _EmailType = value.result![0].provider!;
        a = List.generate(
  ...
  UserEmailCreate(int EmailTypeId, String UserName, String Password) async {
    await _controllerUser.UserEmailCreate(_controllerDB.headers(),
            UserId: _controllerDB.user.value!.result!.id,
      ...
      Fluttertoast.showToast(
          msg: AppLocalizations.of(context)!.anErrorHasOccured + e.toString(),
          toastLength: Toast.LENGTH_SHORT,
          gravity: ToastGravity.BOTTOM,
          timeInSecForIosWeb: 1,
          //backgroundColor: Colors.red,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(
        title: AppLocalizations.of(context)!.emailcreate,
          ...
          ? Container(
              ...
              child: Column(
                children: [
                  ...
                  InkWell(
                    onTap: () async {
                      if (!mailController.text.isBlank! &&
                          !passwordController.text.isBlank!) {
                        await UserEmailCreate(_emailTypeId, mailController.text,
